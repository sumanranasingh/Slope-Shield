...
import { WarningData, getActiveWarnings, getSeverityBadge } from '../../data/warnings'
import {
  AlertTriangle,
  FileText,
  Eye,
...
interface EarlyWarningPanelProps {
  onGenerateReport?: (warning: WarningData) => void
}

export default function EarlyWarningPanel({ onGenerateReport }: EarlyWarningPanelProps) {
  const activeWarnings = getActiveWarnings().slice(0, 4) // Show top 4 on dashboard
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="bg-red-500/20 text-red-400 text-[10px] font-bold px-2 py-0.5 rounded-full border border-red-500/30">
                {getActiveWarnings().length} Active
              </span>
            </div>
        ...
        {activeWarnings.map((warning) => (
          ...
          >
            ...
            <div className="flex flex-wrap items-center justify-between gap-2 mb-2">
              <div className="flex items-center gap-2">
                <span
                  className={`px-2.5 py-0.5 rounded text-[10px] font-extrabold uppercase tracking-wider ${getSeverityBadge(
                    warning.severity
                  )}`}
                >
                  {warning.severity}
            ...
            {/* Trigger (Prompt Specification) */}
            <div className="mb-2">
              <div className="text-[11px] text-navy-400">
                <span className="text-navy-500 font-semibold uppercase tracking-wider text-[10px]">Trigger: </span>
                <span className="text-navy-200">{warning.trigger}</span>
              </div>
            </div>

            {/* Recommended Action (Prompt Specification) */}
            ...
            {/* Buttons: View Location & Generate Report (Prompt Specification) */}
            <div className="flex items-center justify-between pt-1 border-t border-navy-800">
              <span className="text-[10px] text-navy-500">
                Affected Area: <span className="text-navy-300">{warning.affectedArea}</span> (Pop: {warning.affectedPopulation.toLocaleString('en-IN')})
              </span>

              <div className="flex items-center gap-2">
                <Link
                  to={`/locations/${warning.locationId}`}
